            ...
            <View style={{ marginLeft: 12, flex: 1 }}>
              ...
              {!editingMBTI ? (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  <Text>
                    MBTI: <Text style={{ fontWeight: '800' }}>{app.settings.mbti ?? '-'}</Text>
                  </Text>
                  <Pressable onPress={() => setEditingMBTI(true)}>
                    <Text>✏️</Text>
                  </Pressable>
                </View>
              ) : (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  ...
                  <Pressable
                    style={s.btn}
                    onPress={async () => {
                      await app.setSettings({ mbti: mbti.toUpperCase() })
              ...
              renderItem={({ item }) => (
                <View style={s.thumb}>
                  ...
                  <View style={{ flexDirection: 'row', gap: 6, marginTop: 6 }}>
                    <Pressable
                      style={s.btn}
                      onPress={() => Alert.alert('PNG 저장', '스토어 빌드에서 제공 예정')}
                    >
        ...
        <Section title="감정기록 모아보기" subtitle="JSON/CSV 내려받기">
          <View style={{ flexDirection: 'row', gap: 8, justifyContent: 'flex-end' }}>
            <Pressable
              style={s.btn}
              onPress={() =>
                downloadWeb('maeumsee-records.json', app.exportRecordsJSON(), 'application/json')
              }
            >
            ...
            <Pressable
              style={s.btn}
              onPress={() =>
                downloadWeb('maeumsee-records.csv', app.exportRecordsCSV(), 'text/csv')
              }
            >
        ...
        <Section title="화면 잠금" subtitle="생체인증 & PIN(4자리)">
          ...
          <View style={s.row}>
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <Pressable
                onPress={() =>
                  app.setSettings({ lock: { ...app.settings.lock, type: 'biometric' } })
                }
                style={[s.chip, app.settings.lock.type === 'biometric' && s.chipOn]}
              >
                <Text style={[s.chipTxt, app.settings.lock.type === 'biometric' && s.chipOnTxt]}>
                  생체
                </Text>
              </Pressable>
              <Pressable
                onPress={() => app.setSettings({ lock: { ...app.settings.lock, type: 'pin' } })}
                style={[s.chip, app.settings.lock.type === 'pin' && s.chipOn]}
              >
                <Text style={[s.chipTxt, app.settings.lock.type === 'pin' && s.chipOnTxt]}>
                  PIN
                </Text>
              </Pressable>
            </View>
          ...
          {app.settings.lock.type === 'pin' && (
            <View style={s.row}>
              ...
              <TextInput
                ...
                onChangeText={(t) =>
                  app.setSettings({
                    lock: { ...app.settings.lock, pin: t.replace(/[^0-9]/g, '').slice(0, 4) },
                  })
                }
                placeholder="••••"
              />
        ...
        <Section title="계정" subtitle="데이터 초기화">
          <View style={{ flexDirection: 'row', gap: 8, justifyContent: 'flex-end' }}>
            <Pressable
              ...
              onPress={async () => {
                await app.clearAll()
                Alert.alert('로그아웃', '로컬 데이터 초기화 완료')
              }}
            >
              <Text>로그아웃</Text>
            </Pressable>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
        <Pressable
          ...
          onPress={() => {
            ...
            if (Platform.OS === 'web') location.href = url
            else Alert.alert('메일 앱이 열립니다.')
          }}
        >
